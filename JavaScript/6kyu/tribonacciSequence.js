...
3 (instead of 2) numbers of the sequence to generate the next. And, worse part of it,
...
[1, 1] basically shifts the common Fibonacci sequence by once place, you may be tempted
...
then return an empty array (except in C return NULL) and be ready for anything else which is
...
function tribonacci (signature, n) {
  ...
  if (n === 0) return []
  // Using for-loop to finish the tribonnaci sequence
  for (let i = 3; i < n; i++) {
    signature.push(signature[i - 3] + signature[i - 2] + signature[i - 1])
  }
  // Use slice() to return the n-array
